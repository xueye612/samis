<template>
  <ModulePageShell title="液体与血制品字典" description="维护液体和血制品的规格、容量、速度、血型要求、统计分类、适用场景和生命周期" shell-class="config-fluids-page">
    ...
    <a-drawer :visible="editorVisible" :width="560" :title="isCreate ? '新增' : '编辑'" :mask-closable="false" unmount-on-close @cancel="editorVisible = false">
      <a-form :model="form" layout="vertical">
        ...
        <a-row :gutter="12">
          <a-col :span="12"><a-form-item label="默认速度"><a-input v-model="form.defaultRate" /></a-form-item></a-col>
          <a-col :span="12"><a-form-item label="统计分类"><a-input v-model="form.statisticalCategory" /></a-form-item></a-col>
        </a-row>
        <template v-if="activeEntity === 'blood'">
          <a-row :gutter="12">
            <a-col :span="12"><a-form-item label="血型要求"><a-input v-model="form.bloodTypeRequirement" /></a-form-item></a-col>
            <a-col :span="12"><a-form-item label="双人核对"><a-switch v-model="form.doubleCheck" /></a-form-item></a-col>
          </a-row>
        </template>
...
function blank() { const c: Record<string, any> = { code: '', name: '', defaultRate: '', statisticalCategory: '', remark: '', expectedVersion: 1 }; if (activeEntity.value === 'blood') { c.bloodTypeRequirement = ''; c.doubleCheck = true; } return c; }
function openCreate() { isCreate.value = true; Object.assign(form, blank()); editorVisible.value = true; }
function openEdit(r: any) {
  isCreate.value = false;
  const b = blank();
  b.code = r[codeKey()]; b.name = r[nameKey()]; b.defaultRate = r.defaultRate ?? ''; b.statisticalCategory = r.statisticalCategory ?? ''; b.remark = r.remark ?? ''; b.expectedVersion = r.version;
  if (activeEntity.value === 'blood') { b.bloodTypeRequirement = r.bloodTypeRequirement ?? ''; b.doubleCheck = !!r.doubleCheck; }
...
async function onSave() {
  ...
  try {
    ...
    payload.defaultRate = form.defaultRate || null; payload.statisticalCategory = form.statisticalCategory || null; payload.remark = form.remark || null;
    if (activeEntity.value === 'blood') { payload.bloodTypeRequirement = form.bloodTypeRequirement || null; payload.doubleCheck = form.doubleCheck ? 1 : 0; }
    await saveClinicalDictionary(payload); Message.success('保存成功'); editorVisible.value = false; await reload();
